...
function controleerAdmin($connect, $email) {
    ...
    $userRol = $connect -> query($sql); 
    var_dump($userRol); 
    $resultaat = $connect->query("SELECT * FROM user_roles where id = '".$userRol."' and admin=1");
   return (($resultaat -> fetch_assoc()['name']) == "admin")?false:true;
}